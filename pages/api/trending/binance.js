...
export default async function handler(req, res) {
    ...
    if (req.method === "GET") {
        try {
            const coins = await Coin.find({ blockchain: "BSC" });
            ...
            if (stats) {
                // Sort coins by 1-hour percentage change in descending order
                const trendingCoins = stats
                    .map((stat) => ({
                        ...stat,
